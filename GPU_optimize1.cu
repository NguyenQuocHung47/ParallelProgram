...
public:
    ...
    void backward(const float* batch_output_gradients, int batch_size) {
        ...
        backward_kernel2<<<blocks, threads, output_size * sizeof(float)>>>(d_batch_output_gradients, d_weights, d_batch_input_gradients, d_weight_gradients, d_input, d_bias_gradients, input_size, output_size, batch_size);
