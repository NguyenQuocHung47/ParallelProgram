...
// CUDA kernel for backward pass (gradients computation)
__global__ void backward_kernel(const float* output_gradients, const float* weights,
    ...
    if (input_idx < input_size && sample < batch_size) {
        ...
        gradient *= (inputs[sample * input_size + input_idx] > 0) ? 1.0f : 0.0f;//relu_derivative(inputs[sample * input_size + input_idx]);
