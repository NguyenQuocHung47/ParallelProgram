...
// CUDA kernel optimizied use unrolling
__global__ void forward_kernel1(const float* __restrict__ inputs, float* __restrict__ outputs, const float* __restrict__ weights, const float* __restrict__ biases,
    ...
    if (idx < total_outputs) {
        ...
        for (; j <= input_size - 4; j += 4) {
            sum += inputs[sample_idx * input_size + j] * weights[neuron_idx * input_size + j];
            sum += inputs[sample_idx * input_size + j + 1] * weights[neuron_idx * input_size + j + 1];
            sum += inputs[sample_idx * input_size + j + 2] * weights[neuron_idx * input_size + j + 2];
            sum += inputs[sample_idx * input_size + j + 3] * weights[neuron_idx * input_size + j + 3];
        }
...
__global__ void backward_kernel(const float* __restrict__ output_gradients, const float* __restrict__ weights,
                                 float* __restrict__ input_gradients, float* __restrict__ weight_gradients,
                                 float* __restrict__ bias_gradients, const float* __restrict__ inputs,
                                 int input_size, int output_size, int batch_size) {
    ...
    if (input_idx < input_size && sample < batch_size) {
        ...
        for (; i <= output_size - 4; i += 4) {
            float output_gradient1 = output_gradients[sample * output_size + i];
            float output_gradient2 = output_gradients[sample * output_size + i + 1];
            float output_gradient3 = output_gradients[sample * output_size + i + 2];
            float output_gradient4 = output_gradients[sample * output_size + i + 3];

            gradient += output_gradient1 * weights[i * input_size + input_idx];
            gradient += output_gradient2 * weights[(i + 1) * input_size + input_idx];
            gradient += output_gradient3 * weights[(i + 2) * input_size + input_idx];
            gradient += output_gradient4 * weights[(i + 3) * input_size + input_idx];

            // Accumulate weight gradients
            atomicAdd(&weight_gradients[i * input_size + input_idx], output_gradient1 * inputs[sample * input_size + input_idx]);
            atomicAdd(&weight_gradients[(i + 1) * input_size + input_idx], output_gradient2 * inputs[sample * input_size + input_idx]);
            atomicAdd(&weight_gradients[(i + 2) * input_size + input_idx], output_gradient3 * inputs[sample * input_size + input_idx]);
            atomicAdd(&weight_gradients[(i + 3) * input_size + input_idx], output_gradient4 * inputs[sample * input_size + input_idx]);
        }

        // Handle remaining elements
        for (; i < output_size; ++i) {
            float output_gradient = output_gradients[sample * output_size + i];
...
public:
    ...
    void forward(const float* batch_input, int batch_size) {
        ...
        forward_kernel1 << <blocks, threads >> > (d_input, d_output, d_weights, d_biases, input_size, output_size, batch_size);
    ...
    void backward(const float* batch_output_gradients, int batch_size) {
        ...
        backward_kernel << <blocks, threads >> > (d_output_gradients, d_weights, d_input_gradients, d_weight_gradients, d_bias_gradients, d_inputs, input_size, output_size, batch_size);
        CHECK(cudaGetLastError());
        CHECK(cudaDeviceSynchronize());
        CHECK(cudaGetLastError());
...
int main() {
    ...
    nn.save_model("model_GPU_unrolling.bin");
    NeuralNetwork nn1(architecture);
    nn1.load_model("model_GPU_unrolling.bin");
