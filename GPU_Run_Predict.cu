...
int main(int argc, char* argv[]) {
    ...
    const string train_images_path = ""; // Path to the data file
    const string train_labels_path = "";
    string test_images_path = "x_test.csv";
    string test_labels_path = "y_test.csv";
    ...
    test_images_path = "output_data.csv";
    test_labels_path = "";
    num_streams = 1; // Reset number of streams
    // Load and preprocess the dataset
    load_and_preprocess_dataset_csv(train_images_path, train_labels_path, test_images_path, test_labels_path,
        train_images, train_labels, test_images, test_labels,
        train_image_rows, train_label_rows, test_image_rows, test_label_rows,
        image_cols, num_classes);
        float* t_predictions = nn1.forward(test_images, 1);
        int predicted = max_element(t_predictions, t_predictions + num_classes) - t_predictions;
    // Output the label of the first image
        cout << "Label of the first image: " << predicted << endl;
